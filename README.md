...
We would expect that due to `--memory=128M --kernel-memory=16M` the container gets killed due to high usage of kernel memory.

## What happens

The following happens on all tested systems Ubuntu 16.04 LTS (Kernel 4.4.0), Ubuntu 18.04 LTS (Kernel 5.3.0), Ubuntu 20.04 LTS (Kernel 5.4.0-29), Debian, CentOS 8.1 (Kernel 4.18.0). The following screen cast is recorded on CentOS 8.1.

![Showcase of a fork-bomb crash on CentOS 8.1](videos/fork-bomb-success-01.gif)

You can find other recordings of the same scripts with other windows open [in the videos directory](videos).

## Syslog of (another) exemplary run
